...
public OnGameModeInit()
{
    ...
    // Custom skins
    // Put these files inside the server "models" folder.
...
{
    if(!strcmp(cmdtext, "/skin police", true))
    {
        SetPlayerSkin(playerid, SKIN_POLICE);
        SendClientMessage(playerid, 0x00FF00FF, "Custom police skin applied.");
        return 1;
    }

    if(!strcmp(cmdtext, "/skin medic", true))
    {
        SetPlayerSkin(playerid, SKIN_MEDIC);
        SendClientMessage(playerid, 0x00FF00FF, "Custom medic skin applied.");
        return 1;
    }

    if(!strcmp(cmdtext, "/skin off", true))
    {
        SetPlayerSkin(playerid, 0);
        SendClientMessage(playerid, 0xFFFFFFFF, "Custom skin removed.");
        return 1;
    }
...
{
    SendClientMessage(playerid, 0x00FF00FF, "Custom skin files finished downloading.");
    return 1;
}
